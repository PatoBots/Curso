...
// Definindo variaveis para os calculos do controle
long int faixa = 0;
long int ref = 1000;
long int erro = 0;
...
// Definindo a constante proporcional (Kp), e a variavel que velocidade
// que serão fundamentais para o calculo do controle
// São essas variaveis que devemos alterar para encontrar o melhor controle para os robôs
float Kp = 0.1;
float vel = 100;
int PWMD,PWME;
...
void loop() {
  ...
  // Se os valores recebidos dos sensores forem maior que 460, o carrinho não deverá andar
  if((valorA0 > 460) && (valorA1 > 460) && (valorA2 > 460)){
    ...
    // Definimos a velocidade de cada um dos motores
    PWMD=vel-delta;
    PWME=vel+delta;

    // A velocidade não pode ser maior que 255 ou menor que 0
    if (PWMD<0) PWMD=0;
